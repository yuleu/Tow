...
function Baidu(){ //百度音乐
if (DM('download?')){
var Bld=_Q('.login-down'),Bhr=_Q('.high-rate'),Bl=_Qa('li');
function TBurl(Urlc){
...
//登录检查，修复无高品质问题
if((Bld != 'null') || (!win.isLogin=='true')){if (DM('#bai')){Lc.href=BUrl+TBurl(Bl[0])};if(Bld){Bld.style="color:yellow;background-color:gold;font-size:20px;font-weight:bold";}}else{
...
//以下截获Baidu下载事件
if (/\/\d+$/.test(lurl)){
win.ting.media.downloadSong = function (e){
...
_Qa('.playListBtn')[i].innerHTML+='<span id="Yum" style="position:absolute;cursor:pointer;color:blueviolet;margin:-3px;" onclick="location.href=\'http://wma.9ku.com'+Murl+'\'" '+Tit+'>下载<span/>';
}
}
...
function Hcdj(){ //DJ音乐厅
var f39=_Q('table').getElementsByTagName('font')[39];
f39.outerHTML+=' <a href="'+win.s_hcdjstr+win.url+'" '+Tit+Sty0+'>下载 </a>';
}
